...
The application uses a PostgreSQL database for data storage, integrates with OpenAI for embeddings (in the previous PDF GPT context), and employs libraries like Pandas, Scikit-learn, Ultralytics YOLO, and ReportLab for various functionalities.
...
- (Optional) OpenAI API key for any extended functionality from the previous PDF GPT context

## Setup Instructions
1. **Clone or Download the Project**
   - Download the project files (`app.py`, `requirements.txt`, `.env`) to a local directory.
   - Ensure the `templates/` folder contains any required HTML files (e.g., `index.html` from the previous context if used).
   - Place `Roboto-Regular.ttf` and `Roboto-Bold.ttf` in the project directory for PDF generation.
...
4. **Configure Environment Variables**
   ...
   - If integrating OpenAI (from the previous PDF GPT context), add:
     ```
...
5. **Set Up PostgreSQL Database**
   ...
   - Update the database connection string in `app.py` if necessary:
     ...
     ```
     Replace `kaniniintern` with your PostgreSQL password and adjust the host/port as needed.
...
6. **Directory Structure**
   ...
   project_directory/
   ├── templates/
   │   └── index.html  # If used from previous context
...
1. **Start the Flask Application**
   ...
   python app.py
   ```
   This starts the Waitress server (used instead of Flask's default server for production-like deployment) on port 5000 with 4 threads. You should see logs indicating the server is running at `http://0.0.0.0:5000`.

2. **Access the Application**
   Open a web browser and navigate to `http://localhost:5000`. If the `index.html` from the previous PDF GPT context is used, it will display a PDF upload and question-answering interface. Other endpoints are API-based and can be accessed via tools like Postman or cURL.
...
- **Fonts**: The `Roboto-Regular.ttf` and `Roboto-Bold.ttf` files are required for PDF generation. Download them from a reliable source (e.g., Google Fonts) if not provided.
...
- **Security**: The `.env` file contains sensitive information (email credentials). Keep it secure and avoid committing it to version control.
- **Dependencies**: Some packages (e.g., `ultralytics`, `spacy`) may require additional setup (e.g., downloading Spacy models: `python -m spacy download en_core_web_sm`).
...
## License
This project is for educational and professional use. No specific license is provided. Modify and use it as needed for internal or academic purposes.
